...
namespace iih.ci.ord.ciorder.takedrug4outhosp.view
{
    ...
    /// </summary>
    public partial class TakeDrugOutCardView : CiorderBaseControl, IWorkStationRegist
    {
        ...
        /// 医疗单的名称处理
        /// </summary>
        private TakeDrugOutItemView orderItemView;
        /// <summary>
        /// 医疗单来源
        /// </summary>
        ...
        public void SetOrderItemView(TakeDrugOutItemView itemobj)
        {
        ...
        private bool LoadOrderView(string srvTp, MedSrvDO med)
        {
            ...
            if (status != DOStatus.NEW)
            {
                ...
                this.currentEmsControl.EmsHeadDO.Dt_begin_ui = this.NowTime();//获得服务器时间，为开始时间赋值
                this.currentEmsControl.EmsHeadDO.Days_or = 1;
                this.currentEmsControl.EmsHeadDO.Dt_end_ui = ((DateTime)this.currentEmsControl.EmsHeadDO.Dt_begin_ui).AddDays((int)this.currentEmsControl.EmsHeadDO.Days_or);
                this.currentEmsControl.EmsHeadDO.Times_cur = CalQuantumUtil.GetInstance().getTotalTimes(med.Id_freq, this.currentEmsControl.EmsHeadDO.Days_or);
            }
            switch (srvTp.Substring(0, 2))
            {
                ...
                case "01": //药品 
                    ...
                    if (status == DOStatus.NEW)
                    {
                        ...
                        {
                            //获得物品信息
                            this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrug = DrugUtils.getMMOfDrug(med.Id_srv, this.currentEmsControl.EmsHeadDO.PatInfo,true);
                            //执行科室填充
                            ...
                            if (this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrug != null && this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrug.Count > 0)
                            {
                                ...
                                this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrugList[0].Price = LogicEx.GetInstance().getMaterialStocksCount(this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrugList[0].Id_mm, this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrugList[0].Id_unit_sale);
                                this.currentEmsControl.EmsHeadDO.Emsdrugs.Id_dep = this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrugList[0].Id_mp_dep;
                                this.currentEmsControl.EmsHeadDO.Emsdrugs.Name_dep = this.currentEmsControl.EmsHeadDO.Emsdrugs.EmsOrDrugList[0].Name_mp_dep;
                                this.currentEmsControl.EmsHeadDO.Emsdrugs.Use_days = this.currentEmsControl.EmsHeadDO.Days_or;//开立时默认为1
                            }
                    ...
                    this.currentEmsControl.CiEmsDTO.Fg_pres_outp = true;
                    this.currentEmsControl.EmsHeadDO.Emsdrugs.Fg_outp = true;
                    //草药不处理 2016-07-07zwq
