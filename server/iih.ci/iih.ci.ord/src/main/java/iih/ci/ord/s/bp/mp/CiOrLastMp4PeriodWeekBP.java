...
/**
 * 获得频次类型为周期性周时，最后一顿执行判断逻辑数据结果操作BP
 * （不含特例情况处理情形）
 */
public class CiOrLastMp4PeriodWeekBP {
	/**
	 * 获得频次类型为周期性周时
	 * 最后一顿执行判断逻辑数据结果
	 * @param param
	...
	public CiOrLastMpDTO exec(CiOrLastMpDTO param,Hashtable freqHt) throws BizException{
		...
		//最后一顿判断
		if(FDateTime.getHoursBetween(param.getDt_mp_plan(), param.getDt_end())>=CiOrdUtils.HOURS_PER_WEEK){
	...
	private FDateTime[] getDateArray(FDateTime dt_b,FDateTime dt_e,FreqTimeDO[] freqtimedos){
		return OrSrvSplitUtil.getDateArrayByWeek(dt_b.addSeconds(1), dt_e, freqtimedos);
	}
