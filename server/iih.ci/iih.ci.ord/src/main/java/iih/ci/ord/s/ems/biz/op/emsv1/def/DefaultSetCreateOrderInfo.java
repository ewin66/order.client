...
public class DefaultSetCreateOrderInfo extends DefaultSingleSrvCreateOrderInfo {
	...
	protected DefaultBaseCreateOrderInfo createOrderSrvAExtInfo(OrderPackageInfo orderPakageInfo,
		...
		if (CiOrdUtils.isTrue(orderPakageInfo.getOrderInfo().getFg_set())){
			...
			OrderSrvExtPackage osasp = CalcSrvFeesOrdSrvInfo(this.getEnContextInfo(),orderPakageInfo.getOrderInfo(), bdSrvMmInfoList.get(0));
	 ...
	 * 计算费用关联服务项目
	 * 
	 * @param szMedSrvInfo 服务套信息
	 * @return
	 * @throws BizException
	 */
	public OrderSrvExtPackage CalcSrvFeesOrdSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, BdSrvMmInfo bdSrvMmInfo) throws BizException {
		
		// 根据服务套id 查询套内项目
		MedSrvSetItemDO[] szMedSrvSetItemDO = BdSrvInfoUtils.QueryMedSrvSetItemBy(bdSrvMmInfo.getBdSrvInfo().getId_srv(), true);
	 ...
	 * @param bdSrvMmInfo  定义态基础数据服务和物品对象聚集对象
	 * @param szId_srv  套内项目服务id数组
	 * @return
	 * @throws BizException
	 */
	public OrderSrvExtPackage CalcSrvFeesOrdSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, BdSrvMmInfo bdSrvMmInfo,String[] szId_srv) throws BizException {
		MedSrvSetItemDO[] szMedSrvSetItemDO=ServiceFinder.find(IMedSrvSetItemDORService.class).findByAttrValStrings(MedSrvSetItemDO.ID_SRV_ITM, szId_srv);
		return CalcSrvFeesOrdSrvInfo(ctx,orderInfo,bdSrvMmInfo,szMedSrvSetItemDO);
	}
	 ...
	 * @throws BizException
	 */
	public OrderSrvExtPackage CalcSrvFeesOrdSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, BdSrvMmInfo bdSrvMmInfo,MedSrvSetItemDO[] szMedSrvSetItemDO) throws BizException {
		OrderSrvExtPackage ordSrvExtPackage = new OrderSrvExtPackage();
		if (CiOrdUtils.isEmpty(szMedSrvSetItemDO))return ordSrvExtPackage;
		// 单选模式默认选择第一个
		...
		case IBdPrimdCodeConst.ID_PRI_SRV:// 本服务定价
			// 1. 计算本服务定价
			//ordSrvExtPackage = CalcSelfPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo);
			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo,szMedSrvSetItemDO,new SelfSrvCalcPriModeProxy(
					bdSrvMmInfo.getBdSrvInfo().getId_srv(), 
					bdSrvMmInfo.getBdSrvInfo().getName(), 
					szMedSrvSetItemDO.length));
			break;
		case IBdPrimdCodeConst.ID_PRI_SRV_COMP:// 组合定价
//			ordSrvExtPackage = CalcCompPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo); 
			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo,szMedSrvSetItemDO,new CompCalcPriModeProxy(
					bdSrvMmInfo.getBdSrvInfo().getId_srv(), 
					bdSrvMmInfo.getBdSrvInfo().getName(), 
					szMedSrvSetItemDO.length));
			break;
		case IBdPrimdCodeConst.ID_PRI_SRVSET_AMOUNT:// 套成员合计价
			ordSrvExtPackage = CalcSumPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo);
//			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo,new );
			break;
		case IBdPrimdCodeConst.ID_PRI_SRVSET_MU:// 套个数加收
//			ordSrvExtPackage = CalcNumAPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo);
			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo,szMedSrvSetItemDO,new NumACalcPriModeProxy(
					bdSrvMmInfo.getBdSrvInfo().getId_srv(), 
					bdSrvMmInfo.getBdSrvInfo().getName(), 
					szMedSrvSetItemDO.length));
			break;
		case IBdPrimdCodeConst.ID_PRI_SRVSET_FIX:// 套个数定价
//			ordSrvExtPackage = CalcNumPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo);
			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo, szMedSrvSetItemDO, new NumCalcPriModeProxy(
							bdSrvMmInfo.getBdSrvInfo().getId_srv(), 
							bdSrvMmInfo.getBdSrvInfo().getName(), 
							szMedSrvSetItemDO.length));
			break;
	...
	protected OrderSrvExtPackage CalcSumPriOfOrdSrvPackage(CiEnContextDTO ctx, CiOrderDO orderInfo,
		...
		for (int index = 0; index < szMedSrvSetItemDO.length; ++index) {
			...
			if (bdSrvInfoWithSetItemCache.containsKey(mssid.getId_srv_itm())) {
				...
				// 本服务定价
				if (IBdPrimdCodeConst.ID_PRI_SRV.equals(srvInfo.getId_primd())) {

				}
				// 对应物品价格
				else if (IBdPrimdCodeConst.ID_PRI_RES.equals(srvInfo.getId_primd())) {
					
				}
				// 组合计价
				else if (IBdPrimdCodeConst.ID_PRI_SRV_COMP.equals(srvInfo.getId_primd())) {
					// TODO: 成员合计项目中的【组合计价】处理
				}
				// 不付费
				else if (IBdPrimdCodeConst.ID_PRI_FREE.equals(srvInfo.getId_primd())) {

				}
				else if (IBdPrimdCodeConst.ID_PRI_PLUGIN.equals(srvInfo.getId_primd())) {

				}
				
				// 组织服务信息和物品信息
				osmil.add(createOrderSrvMmInfo(ctx,orderInfo,srvInfo,OrSrvSourceFromEnum.PHYSIAN,index));
				
				// 临床项目生成套项目信息
				if (CiOrdUtils.isTrue(mssid.getFg_clinical()) && CiOrdUtils
						.isTrue(mssid.getFg_active())) {
	 ...
	 * @throws BizException
	 */
	private OrderSrvExtPackage CalcPriStdSrvOfOrdSrvList(CiEnContextDTO ctx, CiOrderDO orderInfo, MedSrvDO setSrvInfo,
	 ...
	 * @param ctx
	 * @param orderInfo  医嘱对象
	 * @param bdSrvMmInfo  基础数据和物品数据聚集对象
	 * @return
	 * @throws BizException
	 */
	protected OrderSrvExtPackage CalcPriOfOrdSrvPackage(CiEnContextDTO ctx, CiOrderDO orderInfo,
			BdSrvMmInfo bdSrvMmInfo,MedSrvSetItemDO[] szMedSrvSetItemDO,ICalcPriModeProxy iCalcPriModeProxy) throws BizException {
		
		
		// 根据费用个数定价信息生成医嘱信息
		OrderSrvExtPackage ordSrvExtPackage = CalcPriStdSrvOfOrdSrvList(ctx, orderInfo, bdSrvMmInfo.getBdSrvInfo(), iCalcPriModeProxy);
	...
	protected OrderSrvMmList createOrderSrvMmList(CiEnContextDTO ctx,CiOrderDO orderInfo, MedSrvDO[] szMedSrvInfo, int eu_sourcemd) throws BizException{
		...
		for (MedSrvDO srvInfo : szMedSrvInfo) {
			osmil.add(createOrderSrvMmInfo(ctx,orderInfo,srvInfo,eu_sourcemd,index++));
		}
		return osmil;
	}
	 ...
	 * @throws BizException
	 */
	protected OrderSrvMmInfo createOrderSrvMmInfo(CiEnContextDTO ctx,CiOrderDO orderInfo, MedSrvDO srvInfo, int eu_sourcemd, int index) throws BizException{
		BdSrvMmInfo bdSrvMmInfo = new BdSrvMmInfo(srvInfo);
		OrderSrvMmInfo ordSrvMmInfo = new OrderSrvMmInfo(assembleOrderSrvInfo(ctx, orderInfo, bdSrvMmInfo, index),null);
