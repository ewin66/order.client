 ...
 */
public class SetMmInfoOfOrSrv8EmsBPNew {
	/**
	 * 根据医疗单项目数据设置医嘱项目对应的物品数据信息
	 * @param srvdo
	 * @return
	 */
	public OrdSrvMmDO exec(OrdSrvDO srvdo,EmsOrDrug srvdto) throws BizException{
	 ...
	 * @throws BizException 
	 */
	private void createCiOrdSrvMmDO(OrdSrvMmDO srvmmdo,OrdSrvDO srvdo,EmsOrDrug srvdto,int status) throws BizException{
		...
		if(CiOrdUtils.isEmpty(srvdto.getPrice())){
			...
			srvmmdo.setPrice_pgku_cur(srvdto.getPrice());
		}
...
//		srvmmdo.setQuan_bed_medu(srvdto.getQuan_bed_medu());//???床边量
...
//		srvmmdo.setDays_available(srvdto.getDays);//领可用天数（门诊用）//???
