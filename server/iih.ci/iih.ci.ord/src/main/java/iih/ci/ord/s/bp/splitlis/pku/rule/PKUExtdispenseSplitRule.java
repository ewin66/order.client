 ...
 *
 */
public class PKUExtdispenseSplitRule implements ICiOrPresSplitRule {

	List<CiOrPresSplitList> outList;
	public Map<String, List> extdispenseMap = new HashMap<String, List>();
	...
	private void AnalyzeOrderPresSplitDTO(List<OrderPresSplitDTO> orderList) {
		for (OrderPresSplitDTO dto : orderList) {
			...
			if (dto.getFg_extdispense()==FBoolean.TRUE) {
				if (extdispenseMap.containsKey(FBoolean.TRUE.toString())) {
					List<OrderPresSplitDTO> list1 = extdispenseMap.get(FBoolean.TRUE.toString());
					list1.add(dto);
					extdispenseMap.put(FBoolean.TRUE.toString(), list1);
			...
			} else if(dto.getFg_extdispense()==FBoolean.FALSE){

				if (extdispenseMap.containsKey(FBoolean.FALSE.toString())) {
					List<OrderPresSplitDTO> list1 = extdispenseMap.get(FBoolean.FALSE.toString());
					list1.add(dto);
					extdispenseMap.put(FBoolean.FALSE.toString(), list1);
	...
	private List<CiOrPresSplitList> getOrderPresSplitList(CiOrPresSplitList ps) {
		...
		if (this.extdispenseMap == null)
			return null;
		Iterator entrys = this.extdispenseMap.entrySet().iterator();
		while (entrys.hasNext()) {
			...
			if (orderList != null && orderList.size() >0) {
				...
				if(key.equalsIgnoreCase("Y")){
					presSplitList.setSd_prestpword(ps.getSd_prestpword()+","+PKUSplitConst.SD_PRESCRIPTION_FLAG_WPY);
					presSplitList.setId_prestpword(ps.getId_prestpword()+","+PKUSplitConst.ID_PRESCRIPTION_FLAG_WPY);
				}else{
