...
/////////添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		// 频次
		orderInfo.setId_freq(drugItemDO.getId_freq());
		orderInfo.setFreq_name(drugItemDO.getName_freq());
		orderInfo.setFreqct(drugItemDO.getFreqct());
		orderInfo.setSd_frequnitct(drugItemDO.getSd_frequnitct());
		// 剂量
		orderInfo.setQuan_medu(drugItemDO.getQuan_med());
		orderInfo.setId_unit_med(drugItemDO.getId_unit_med());
		orderInfo.setName_unit_med(drugItemDO.getName_unit_med());
		// 使用天数
		orderInfo.setDays_or(drugItemDO.getUse_days());
		// 计算总次数
...
/////////添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		...
		orderInfo.setFuncclassstr(drugItemDO.getFuncclassstr());
		orderInfo.setId_srvof(drugItemDO.getId_srvof());
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx, OrdSrvDO srvInfo, Object uiModel) throws BizException {
		...
		if (srvInfo.getId_srv() != null && srvInfo.getId_srv().equals(idSrv)) {
			if (drugInfo.getStatus() == DOStatus.DELETED) {
				...
				srvInfo.setEu_blmd(drugInfo.getEu_blmd());
		...
		/////////添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
				// 频次
				srvInfo.setId_freq(drugInfo.getId_freq());
				srvInfo.setFreq_name(drugInfo.getName_freq());
				srvInfo.setFreqct(drugInfo.getFreqct());
				srvInfo.setSd_frequnitct(drugInfo.getSd_frequnitct());
				// 剂量
				srvInfo.setQuan_medu(drugInfo.getQuan_med());
				// 总量
				srvInfo.setQuan_total_medu(drugInfo.getQuan_cur());
	...
	protected void afterMergeOrderSrvInfo(CiEnContextDTO ctx, final CiOrderDO orderInfo, OrdSrvDO srvInfo, Object uiModel)
			throws BizException {
		EmsOrDrug drugItemDO = (EmsOrDrug)uiModel;
		CiOrDtLastBlCal4OpenBP bp=new CiOrDtLastBlCal4OpenBP();
		srvInfo.setDt_last_bl(bp.exec(srvInfo.getId_freq(),orderInfo.getDt_effe(),null,drugItemDO.getFg_long()));
	}
