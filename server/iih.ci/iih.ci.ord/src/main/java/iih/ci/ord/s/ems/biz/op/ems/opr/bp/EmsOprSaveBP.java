package iih.ci.ord.s.ems.biz.op.ems.opr.bp;
 ...
 *
 */
public class EmsOprSaveBP extends EmsSingleSaveBP {
		...
		for (OrderPackageInfo opInfo : szOrderPackageInfo) {
			...
			if (!CiOrdUtils.isEmpty(opInfo.getOrderAppSheetList()) ) {
				...
				if (objAppSheetInfo instanceof CiorappsurgeryAggDO) { // 手术申请单保存
					((CiorappsurgeryAggDO) objAppSheetInfo).getParentDO().setId_or(id_or);
	...
	protected void mergeOrderInfo(CiEnContextDTO ctx,CiOrderDO orderInfo, Object uiModel) throws BizException{
		...
		OrContentDTO orContentDTO = new OrContentDTO(ems.getName_srv()+CiOrdUtils.COMMA_STR+ems.getName_opex_srvs(), null, null, false);
		orContentDtos.add(orContentDTO);
		OprOrContentParam param = new OprOrContentParam(orderInfo.getSd_srvtp(),ems.getDt_plan(),orderInfo.getRoute_name(),ems.getDes(),orContentDtos);
...
//		ordSrvInfo.setSd_hpsrvtp(Sd_hpsrvtp);
	...
	private void ordOpInfoFrom(EmsOpitemDO ordop,OrdApOpDO opInfo){
		...
		opInfo.setAnnouncements(ordop.getDes());
		opInfo.setName_lvlsug(ordop.getName_lvlsug());
		opInfo.setStatus(ordop.getStatus());
		//		opInfo.setFg_prn();
		//		opInfo.setCnt_prn();
	...
	private void getExOprSrvInfo(CiEnContextDTO ctx,String id_opex_srvs,OrderPackageInfo pInfo) throws BizException{
		// 查询附加术服务信息
