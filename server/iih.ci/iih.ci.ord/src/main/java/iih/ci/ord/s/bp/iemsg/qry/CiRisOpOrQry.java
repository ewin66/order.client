	 ...
	 private String getSql() throws BizException{
		 ...
		 if(str_idor_ris.length()>0){
			 if(formatsql.length()>0)
				 formatsql+=" Union All ";
			 formatsql+=String.format(ICiIEMsgRelSqlConst.CI_IE_APPRIS_OP_ALL_SQL, "A.id_or"+CiOrdUtils.getSqlInOrEqualStrs(str_idor_ris.substring(0, str_idor_ris.length()-1)));
		 }
