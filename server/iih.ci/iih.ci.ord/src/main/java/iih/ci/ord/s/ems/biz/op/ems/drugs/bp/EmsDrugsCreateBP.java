package iih.ci.ord.s.ems.biz.op.ems.drugs.bp;
...
import iih.ci.ord.s.ems.biz.utils.enums.EmsConstEnum;
...
public class EmsDrugsCreateBP extends EmsBaseCreateBP{
	...
	public EmsRstDTO[] create(EmsCrtDTO[] emsarray)  throws BizException {
		...
		OrdBizLogger.info(ems.getEnContext(), "EmsDrugsCreateBP.create.ems——"+(CiOrdUtils.isEmpty(ems)?"":ems.toString()));
		...
		if (null == wf) {
			...
			errorlist.add(String.format("【%s】获取执行科室失败", aggDO.getParentDO().getName()));
		...
		EmsOrDrug drug = this.drugInfoFrom(ems.getId_srv(), ems.getId_mm(), aggDO.getParentDO().getName(), ems.getEnContext());
		{
			...
			// 设置物品总价
			if(!CiOrdUtils.isEmpty(aggDO.getParentDO().getSd_srvtp()) && aggDO.getParentDO().getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)){
				drug.setTotalprice((drug.getPrice().multiply(drug.getQuan_cur())).multiply(new FDouble(EmsConstEnum.HBORDERS)));
			}else{
				drug.setTotalprice(drug.getPrice().multiply(drug.getQuan_cur()));
			}
			// 药品里边集合开立时只有一个
			//抗生素
            if (drug.getFg_anti()!=null&&drug.getFg_anti() == FBoolean.TRUE)
            {
                if (CiOrdUtils.isEmpty(drug.getFg_propc()))
                {
                	drug.setFg_propc(FBoolean.FALSE); // emsordrug.Fg_propc 可能存在空值
                }          
            }
		...
		OrdBizLogger.info(ems.getEnContext(), "EmsDrugsCreateBP.create.rst"+(CiOrdUtils.isEmpty(rst)?"":rst.toString()));
		rsts.add(rst);
		return (EmsRstDTO[]) rsts.toArray(new EmsRstDTO[rsts.size()]);
	}
...
//		emsDrugItem.setDt_end_ui(new FDateTime(emsDrugItem.getDt_begin_ui().getBeginDate().getDateAfter(1), emsDrugItem.getDt_begin_ui().getUFTime()));
	...
	protected EmsOrDrug drugInfoFrom(String id_srv, String id_mm, String name_srv, CiEnContextDTO ctx) throws BizException{
		String cacheKey = String.format("EmsOrDrug{%s-%s}", id_srv, id_mm);
		OrdBizLogger.info(ctx, "EmsDrugsCreateBP.drugInfoFrom:一级缓存key值["+cacheKey+"]");
		EmsOrDrug drug = getCache(L1SessionKeyWith(ctx), cacheKey);
		OrdBizLogger.info(ctx, "EmsDrugsCreateBP.drugInfoFrom:缓存中取出drug["+(!CiOrdUtils.isEmpty(drug)?drug.toString():"")+"]");
        ...
        OrdBizLogger.info(ctx, "EmsDrugsCreateBP.drugInfoFrom:null==drug = true");
			...
			drug = (EmsOrDrug)daf.execQuery(sqlSB.toString(), param, new BeanHandler(EmsOrDrug.class));
		OrdBizLogger.info(ctx, "EmsDrugsCreateBP.drugInfoFrom:sql["+sqlSB.toString()+"]/n,id_srv["+id_srv+"],id_mm["+id_mm+"]\n "+"待放入缓存drug:\n "+(!CiOrdUtils.isEmpty(drug)?drug.toString():""));
		putCache(L1SessionKeyWith(ctx), cacheKey, drug);
		}
		// 检验是否绑定物品
		...
		if (CiOrdUtils.isHpUsing(ctx) && !FBoolean.TRUE.equals(cusFg_selfpay)){
			...
			if (tmpBdHpIndicationDTOCache != null && tmpBdHpIndicationDTOCache.size() > 0 ) {
				...
				if(hpIndiccation.getId_hpsrvtp().equals(IBdPpCodeTypeConst.ID_HP_BJ_THREE) && !CiOrdUtils.isTrue(hpIndiccation.getFg_indic())){
		...
		IMaterialBaseInfoService materService = ServiceFinder.find(IMaterialBaseInfoService.class);
		{
			...
			cacheKey = String.format("MaterialUnitDTO[]{%s-%s}", ctx.getCode_entp(),id_mm);
			MaterialUnitDTO[] szMaterUnit = getCache(L1SessionKeyWith(ctx),cacheKey);
			if (null == szMaterUnit){
				szMaterUnit = materService.getMMunitByEntp(new String[]{id_mm}, ctx.getCode_entp());
				putCache(L1SessionKeyWith(ctx),cacheKey,szMaterUnit);
			}
