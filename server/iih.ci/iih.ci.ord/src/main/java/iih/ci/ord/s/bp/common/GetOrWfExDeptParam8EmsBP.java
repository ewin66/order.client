 ...
 */
public class GetOrWfExDeptParam8EmsBP {
	/**
	 * 根据EMS获得医嘱流向执行科室入口参数
	 * @param paramdto
	 * @return
	 * @throws BizException
	 */
	public  OrWfExDeptParamDTO exec(CiEmsDTO ems,int index,FDateTime dt_effe)  throws BizException{
		//有效性校验
		if(CiOrdUtils.isEmpty(ems) || CiOrdUtils.isEmpty(ems.getEmssrvs()))return null;
			
		//获得执行科室计算参数数据
		ExeDeptCalParamDTO calparamdto=getExeDeptCalParamDTO(ems,index,dt_effe);
	 ...
	 * @throws BizException 
	 */
	private OrWfExDeptParamDTO getOrWfExDeptParam(ExeDeptCalParamDTO param) throws BizException{
