...
public class CiOrderCopyBP {
	...
	public CiOrderCopyBP(String[] idOrs, CiEnContextDTO ciEnContext) {
	...
	public MoreEmsParamDTO exec() throws BizException {
		...
		return ciOrTmpl.Mapping(ciEnContext, ortmpls);
		...
		MoreEmsParamDTO paramDTO = SaveCiEmsDTOBP.SaveCiEmsDTO(ciEnContext, ciEmsDTOs);
		return paramDTO;*/
	}
	...
	private CiOrTmplDTO[] getCiOrderCopys() throws BizException {
		...
		for (int i = 0; i < ciorderAggs.length; i++) {
			// 复制医嘱
			ciorTmpls[i] = this.getCiOrTmplCopy(ciorderAggs[i]);
		}
	...
	private void copyCiOrTmpl(CiOrderDO source, CiOrTmplDTO target) {
		...
		// target.setCode(source.getCode_or());// 编码 CiOrderDO中code_or是用于集成平台生成的随机串，不是bd_srv中的编码
