...
import iih.ci.ord.s.ems.biz.op.ems.ris.EmsRisValidate;
...
//	}
	...
	protected void mergeOrderInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, Object uiModel) throws BizException {
		...
		orderInfo.setContent_or(CiOrContentUtils.create(orderInfo.getSd_srvtp(), emsObsItemDO.getName_srv(),
				orderInfo.getRoute_name(), emsObsItemDO.getFg_urgent()).toString());
		orderInfo.setApplyno(emsObsItemDO.getNo_applyobs());
		...
		if (orderInfo.getFg_mp_in() == FBoolean.TRUE) {
...
/////////添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		// 频次
		orderInfo.setId_freq(emsObsItemDO.getId_freq());
		orderInfo.setFreq_name(emsObsItemDO.getName_freq());
		orderInfo.setFreqct(emsObsItemDO.getFreqct());
		orderInfo.setSd_frequnitct(emsObsItemDO.getSd_frequnitct());
		// 剂量
		orderInfo.setQuan_medu(emsObsItemDO.getQuan_med());
		orderInfo.setId_unit_med(emsObsItemDO.getId_unit_med());
		orderInfo.setName_unit_med(emsObsItemDO.getName_unit_med());
		// 使用天数
...
/////////添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx, OrdSrvDO srvInfo, Object uiModel) throws BizException {
		...
		if (FBoolean.TRUE.equals(srvInfo.getFg_set())) {
			...
			srvInfo.setId_srv_src(srvdto.getId_srv());
...
//		srvInfo.setStatus(srvdto.getStatus());
	...
	protected void mergeOrderSetInfo(CiEnContextDTO ctx, OrdSrvSetDO[] setSrvList, Object uiModel) throws BizException {
		...
		if (!CiOrdUtils.isEmpty(setSrvList)) {
			EmsObsItemDO emsObsItemDO = (EmsObsItemDO) uiModel;
			FArrayList opList = emsObsItemDO.getEmsOrObsListEx();
			for (OrdSrvSetDO setSrvDo : setSrvList) {
				for (Object object : opList) {
					EmsObsLap emsObsLap = (EmsObsLap) object;
					if(emsObsLap.getId_srv().equals(setSrvDo.getId_srvset())){
						setSrvDo.setId_body(emsObsLap.getId_body());
						setSrvDo.setSd_body(emsObsLap.getSd_body());
						setSrvDo.setBody_name(emsObsLap.getName_body());
					}
				}
			}
		}
