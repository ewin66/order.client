...
public class CiOrAggAndRelInfoSaveBPNew {
	 ...
	 * @throws BizException
	 */
	public CiOrderTransMissionDTO exec(CiOrAggAndRelDatum oraggandrelinfo,boolean iseventsave,int iemstp) throws BizException{
		if(oraggandrelinfo==null || oraggandrelinfo.getOraggdo()==null)return null;
		CiOrderTransMissionDTO  transMissionDTO = new CiOrderTransMissionDTO();  
		//保存之前的验证  执行科室
		String medicalinfo ="";
			...
			if (!OrSourceFromEnum.IIHSRVCAHELPER.equals(eu_orsrcmdtp)
				...
				if (result != null && !result.getCheckflag().booleanValue()) {
					...
					if(context != null  && !IEnDictCodeConst.SD_ENTP_INPATIENT.equals(context.getCode_entp())){
						...
						transMissionDTO.setCiencontextdto(context);
						list.add(oraggandrelinfo.getOraggdo());
						transMissionDTO.setCiorderaggdo(list);
						transMissionDTO.setCiemstype(iemstp);
						return transMissionDTO; //oraggandrelinfo.getOraggdo();
					}
				}
			}
		}
