 ...
 * 临床检验医嘱标本采集时间分单规则
 */
public class CiLisOrSmsSampColTimeRule implements ICiLisOrSmsRule {
	...
	private Hashtable<String, FArrayList2> AnalyzeLisOrInfo4Sms(FArrayList2 lisOrInfo4SmsList) {
		...
		for (Object d : lisOrInfo4SmsList) {
			...
			if(dto.getOrlisapdo().getId_sampcoltime()==null){
				...
				if (htors8lis.containsKey(dto.getOrlisapdo().getId_sampcoltime())) {
					FArrayList2 list = htors8lis.get(dto.getOrlisapdo().getId_sampcoltime());
					list.add(dto);
				} else {
					FArrayList2 list = new FArrayList2();
					list.add(dto);
					htors8lis.put(dto.getOrlisapdo().getId_sampcoltime(), list);
				}
	...
	private void getLisOrSmsIoList(CiLisOrSmsIoDTO sms,Hashtable<String, FArrayList2> htors8lis) {
		...
		while (entrys.hasNext()) {
            ...
            smsio.setId_sampcoltime(dto.getOrlisapdo().getId_sampcoltime());
