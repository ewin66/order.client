...
public class MedSrvOpImportServiceImpl extends BaseDataImport<MedSrvOpDO>
		implements IDataImport {

	@Override
	public boolean checkNotNull() throws BizException {
		// 1、从导入临时库得到需要导入目标库的数据,此数据是数据源为 import 的库获得
		MedSrvOpDO[] dos = getDOsFromImportDB(new MedSrvOpDO(),"1=1",false);
	...
	public boolean importData() throws BizException {
		// 1、从导入临时库得到需要导入目标库的数据,此数据是数据源为 import 的库获得
		MedSrvOpDO[] dos = getDOsFromImportDB(new MedSrvOpDO(),"1=1",true);
    	if (ArrayUtils.isEmpty(dos))
    		throw new BizException("医疗服务导入数据为空！");
		...
		ServiceFinder.find(IMedSrvOpDOCudService.class).insert(dos);
	...
	public boolean cleanData() throws BizException {
		...
		daFacade.execUpdate("delete from "+new MedSrvOpDO().getTableName()+""+ " where "+new MedSrvOpDO().getPKFieldName() +" not like '@@@@%'");
		return true;
	}

	@Override
	public String[] getImportTable() throws BizException {
		return new String[]{new MedSrvOpDO().getTableName()};
	}
}
