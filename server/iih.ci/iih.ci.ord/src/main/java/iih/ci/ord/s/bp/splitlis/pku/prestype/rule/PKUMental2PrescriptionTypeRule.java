 ...
 *
 */
public class PKUMental2PrescriptionTypeRule implements ICiOrPresSplitRule {
	...
	private void AnalyzeOrderPresSplitDTO(List<OrderPresSplitDTO> orderList) {
		...
		for (OrderPresSplitDTO dto : orderList){
			if (dto.getSd_pois() != null && IBdMmDictCodeConst.SD_POIS_JINGSHEN_2.equals(dto.getSd_pois())) {
				listOP.add(dto);	 
			}else{
			    list1.add(dto);
				}
		...
		if(listOP != null && listOP.size()>0 ){
			hpMap.put(IBdMmDictCodeConst.SD_POIS_JINGSHEN_2, listOP);
		}
	...
	private List<CiOrPresSplitList> getOrderPresSplitList(CiOrPresSplitList ps) {
		...
		while (entrys.hasNext()) {
			...
			if (orderList != null && entry.getKey().equals(IBdMmDictCodeConst.SD_POIS_JINGSHEN_2)) {
				...
				if(ps.getName() != null){
					...
					presSplitList.setId_prestp(PKUSplitConst.ID_UDIDOC_PKU_PRESCRIPTION_MZELJSYP);
					presSplitList.setCode(ps.code);
					presSplitList.setName(PKUSplitConst.NAME_UDIDOC_PRESCRIPTION_MZELJSYP);
					presSplitList.setSd_pres(PKUSplitConst.SD_UDIDOC_PRESCRIPTION_MZELJSYP);
					presSplitList.setSd_prestpword(PKUSplitConst.SD_PRESCRIPTION_FLAG_JE);
					presSplitList.setId_prestpword(PKUSplitConst.ID_PRESCRIPTION_FLAG_JE);	
				}
