 ...
 * 回退用血医嘱侦听器插件
 */
public class UseBtOrRollBackListener extends AbstractOrCancelCheckListener {
	...
	protected void doYourActionAfterOrSign(CiOrderDO[] ors) throws BizException {
		...
		for(OrdApBtDO apbtdo : apbtArray){
			apbtdo.setNum_margin_bu(new FDouble((apbtdo.getNum_margin_bu().toDouble()+map.get(apbtdo.getId_or()).toDouble())));
