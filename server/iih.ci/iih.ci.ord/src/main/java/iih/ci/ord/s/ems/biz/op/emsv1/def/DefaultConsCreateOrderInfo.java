...
public class DefaultConsCreateOrderInfo extends DefaultBaseCreateOrderInfo {
	...
	public OrderPackageInfo[] createOrderPakageInfo(CiEnContextDTO ctx, DefaultCreateParam[] szParam) throws BizException {
		...
		for (DefaultCreateParam p : szParam){
			listId_srv.add(p.getBdSrvMmInfoList().get(0).getBdSrvInfo().getId_srv());
		}
...
//		consdo.setCnt_prn();
	...
	private MedSrvConsDO[] getMedsrvConsdo(String[] id_srvs) throws BizException{
		StringBuilder idsrvs=new StringBuilder();
		for(String id_srv:id_srvs){
			idsrvs.append(String.format("'%s',", id_srv));
		}
		return ServiceFinder.find(IMedSrvConsDORService.class).find(String.format(" id_srv in (%s)", idsrvs.substring(0, idsrvs.length()-1)), null, FBoolean.FALSE);
	}
