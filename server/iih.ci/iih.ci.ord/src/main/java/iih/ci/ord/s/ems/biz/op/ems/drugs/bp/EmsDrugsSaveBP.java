 ...
 *
 */
public class EmsDrugsSaveBP extends EmsMedicinesSaveBP {

	public EmsDrugsSaveBP() {
		this.setEmsValidate(new EmsDrugsValidate());
		this.setDefaultCreateOrderInfo(new DefaultDrugsCreateOrderInfo());
	}

	public EmsDrugsSaveBP(IEmsValidate emsValidate) {
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx, OrdSrvDO srvInfo, Object uiModel) throws BizException {
		...
		if (srvInfo.getId_srv() != null && srvInfo.getId_srv().equals(idSrv)) {
			if (curDrug.getStatus() == DOStatus.DELETED) {
				...
				//支付方式
				srvInfo.setEu_blmd(curDrug.getEu_blmd());
...
//				int[] numben = OrSrvSplitUtil.getNumDen(targetDrug.getQuan_med(), targetDrug.getFactor_mb());
	...
	protected String getNameOR(String sd_srvtp, String[] szName) {
		String name_or = "";
		if (!CiOrdUtils.isEmpty(szName)) {
			if (sd_srvtp.equals(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG_ZX)|| 
					sd_srvtp.equals(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG_DSY) || 
					sd_srvtp.equals(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG_KF)||
					sd_srvtp.startsWith( IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)) {
				for (String name : szName) {
