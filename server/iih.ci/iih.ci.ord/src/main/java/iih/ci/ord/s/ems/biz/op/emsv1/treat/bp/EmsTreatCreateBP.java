...
//		rst.setEmsType(EmsType.COMMON);
	...
	private void constructTableDatasource(CiEnContextDTO ctx, EmsDrugItemDO drugItem, MedsrvAggDO medSrvAggInfo)
		...
		//FIXME 需要PO确定启用医保判断的方法,并将医保计划等信息设置到drug对象中
		if (CiOrdUtils.isHpUsing(ctx) && !FBoolean.TRUE.equals(cusFg_selfpay)){
			Map<String,BdHpIndicationDTO> tmpBdHpIndicationDTOCache = HpService.getMedSrvHpService(new MedSrvHpParam[]{ 
					new MedSrvHpParam(med.getId_srv(),null)}, ctx);
			if (tmpBdHpIndicationDTOCache != null && tmpBdHpIndicationDTOCache.size() > 0 ) {
				...
				ems.setFg_treat(hpIndiccation.getFg_indic());
				ems.setFg_selfpay(ems.getSd_srvtp() != null && ems.getSd_srvtp().startsWith("0103")?FBoolean.FALSE:
					(CiOrdUtils.isTrue(hpIndiccation.getFg_indic())?FBoolean.FALSE:FBoolean.TRUE));
				ems.setLimit(hpIndiccation.getDes_hplimit());
				ems.setId_hp(ctx.getId_hp());
				ems.setSd_hpsrvtp(hpIndiccation.getSd_hpsrvtp());
				ems.setId_hpsrvtp(!CiOrdUtils.isEmpty(ems.getSd_hpsrvtp())?CiOrdUtils.idHpSrvtpFromSdHpSrvtp(ems.getSd_hpsrvtp()):null);
				ems.setFg_hpindicjudged(CiOrdUtils.getFg_hpindicjudged(hpIndiccation));
				ems.setBdHpIndicationDTO(new FArrayList().append(hpIndiccation));
			}
		}else{//非医保适应症时，自费标识应该为true
			ems.setFg_selfpay(FBoolean.TRUE);
			ems.setFg_hpindicjudged(0);
			ems.setId_hpsrvtp(IBdPpCodeTypeConst.ID_HP_BJ_THREE);
			ems.setSd_hpsrvtp(IBdPpCodeTypeConst.SD_HP_BJ_THREE);
		}
		// 获取执行科室
