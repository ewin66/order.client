 ...
 */
public class getSrvortplitemAggDOSBP  extends CacheContext {
	private CiEnContextDTO ctx;
	 
	  private static Map<String,MedSrvSetItemDO[]>  MedSrvSetItemDOMap = new HashMap<String,MedSrvSetItemDO[]>();//套项目缓存
	  ...
	  public getSrvortplitemAggDOSBP(CiEnContextDTO ctx)throws BizException{
		  this.ctx = ctx;
	  }
	  /**
       *zwq添加缓存模板数据时使用
	  ...
	  public FMap getNewOrderTemplateDTO2()throws BizException{
		  this.templateList = new FArrayList();
		  this.infoList = new ArrayList();
		  String whereStr =" 1=1";
		  if(getEnTp(ctx.getCode_entp()) != null ){
			  whereStr += "  and " +getEnTp(ctx.getCode_entp()); 
		  }
		  OrtmplAggDO[] aggdo = CiOrdAppUtils.getIOrtmplRService().find(whereStr, OrTmplDODesc.PRIMARYKEY_FIELDNAME, FBoolean.FALSE);

		  return HandleOrTmplate(aggdo,ctx.getEnt4BannerDTO().getId_pripat(),ctx.getId_hp(),ctx.getCode_entp());
	  ...
	  public FMap getNewOrderTemplateDTO2(String[] id_ortmpl,String Id_pripat,String id_hp,String code_entp)throws BizException{
		    ...
		    this.templateList = new FArrayList();
			  ...
			  OrtmplAggDO[] aggdo = CiOrdAppUtils.getIOrtmplRService().find(whereStr, OrTmplDO.PYCODE, FBoolean.FALSE);
			  //医嘱模板项目排序
			  SortNOOrtmplItem(aggdo);
			  CiOrdUtils.LogerOutInfo(" CiOrdAppUtils.getIOrtmplRService().find 模板项目耗时:"+(System.currentTimeMillis()-start2));
			  map = HandleOrTmplate(aggdo,Id_pripat,id_hp,code_entp);
			  CiOrdUtils.LogerOutInfo("类getNewOrderTemplateDTO2 模板项目耗时:"+(System.currentTimeMillis()-start));
			  return map;
			 //return (NewOrderTemplateDTO[]) this.templateList.toArray(new NewOrderTemplateDTO[0]);
	  }
	  public FMap getNewOrderTemplateDTO2(String[] id_ortmpl)throws BizException{
			return getNewOrderTemplateDTO2(id_ortmpl,ctx.getEnt4BannerDTO().getId_pripat(),ctx.getId_hp(),ctx.getCode_entp());
	  }
	  /**
	   * 医嘱项目排序
	   * @param aggdo
	   */
	  public  void SortNOOrtmplItem( OrtmplAggDO[] aggdo){
		  if(aggdo != null && aggdo.length >0){
			for(OrtmplAggDO ormtplDO: aggdo){
				if(ormtplDO != null && ormtplDO.getOrTplNItmDO().length >0){
					ormtplDO.setOrTplNItmDO(selectSort(ormtplDO.getOrTplNItmDO()));
				}
			}
	  ...
	  public   OrTplNItmDO[] selectSort( OrTplNItmDO[] ortmplItem) {
	        for (int x = 0; x < ortmplItem.length - 1; x++) {
	            for (int y = x + 1; y < ortmplItem.length; y++) {
	            	if(ortmplItem[x].getSortno() == null) break;
	                if (ortmplItem[x].getSortno() > ortmplItem[y].getSortno()) {
	                	OrTplNItmDO temp = ortmplItem[x];
	                    ortmplItem[x] = ortmplItem[y];
	                    ortmplItem[y] = temp;
	                }
	            }
	        }
	  ...
	  private FMap HandleOrTmplate(OrtmplAggDO[] aggdo,String Id_pripat,String id_hp,String code_entp)throws BizException{
		  ...
		  HandleFMap2(map,Id_pripat,id_hp,code_entp);
		   //设置价格 医保和服务的启停
		  setvalue(map);
		  CiOrdUtils.LogerOutInfo("类HandleOrTmplate  耗时:"+(System.currentTimeMillis()-start));
		  return map;
	  }	   
		  ...
		  if(aggdo != null && aggdo.getOrTplNItmDO().length >0){
			  ...
			  for(OrTplNItmDO  item:orTplItem){
				if(OrTplItmTypeEnum.ORTMPL.equals(item.getEu_ortplitmtp())){
					 ...
					 //排序
					 SortNOOrtmplItem(new OrtmplAggDO[]{aggdo});
		   ...
		   private void setOrTemplateCYFJ(OrtmplAggDO aggdo)throws BizException{
			   if(aggdo != null && aggdo.getOrTplNItmDO().length >0){
				   ...
				   NewOrderTemplateDTO  template = new NewOrderTemplateDTO();
					...
					template.setName_routedes(aggdo.getParentDO().getOrtmpl_routedes_name());
					template.setFreqdefdo(this.getFreqMap());
					template.setMeasdocdo(this.getMeasDocMap());
					template.setRoutedo(this.getRouteMap());
					template.setBoildo(this.getBoilMap());
					for (OrTplNItmDO item : orTplItem) {
...
//					 template.setName_freq(ortmpl.getOrtplnitm_freq_name());
	  ...
	  private FArrayList getOrTplNItmDOSet(OrTplNItmDO ortmpl)throws BizException{
		  ...
		  //id_srv 是套的 id
		  MedSrvSetItemDO[] rtn;
		  if(MedSrvSetItemDOMap != null && MedSrvSetItemDOMap.containsKey(ortmpl.getId_srv())){
			 rtn =  MedSrvSetItemDOMap.get(ortmpl.getId_srv());
		  }else{
			   GetMedSrvItemDOBP qry = new GetMedSrvItemDOBP(ortmpl.getId_srv());
			   rtn = (MedSrvSetItemDO[])AppFwUtil.getDORstWithDao(qry, MedSrvSetItemDO.class);
			   MedSrvSetItemDOMap.put(ortmpl.getId_srv(), rtn);
		  }
	...
	public OrderTemplateDTO getSrvortplitemAggDOS(String id_srvortpl) throws BizException{
		if(id_srvortpl == null ) throw new  BizException("parameter :id_srvortpl is null ");
	     ...
	     orderTemplateDto.setFreqdefdo(this.getFreqMap());
	     orderTemplateDto.setMeasdocdo(this.getMeasDocMap());
  ...
  private FBoolean setFgSetCheck(SrvortplitemAggDO item,String id_srv){
	...
	public FMap getFreqMap() throws BizException {
		FMap rstFreqInfoMap = getCache(L3SessionKeyWith(ctx),FreqDefList_Cache_Key);
		if (null == rstFreqInfoMap){
			rstFreqInfoMap = new FMap();
			FreqDefDO[] szFreqDefInfo = ServiceFinder.find(IFreqdefMDORService.class).find(" fg_use_op='Y' ", "Name", FBoolean.FALSE);
			if(szFreqDefInfo != null ){
				// 存储频次行数据
				FArrayList listFreqDefInfo =  new FArrayList();
				for(FreqDefDO freq:szFreqDefInfo){
					...
					listFreqDefInfo.add(newFreq);
				}
				rstFreqInfoMap.put("FreqDefDO", listFreqDefInfo);
	...
	public FMap getBoilMap() throws BizException {
	 ...
	 * @throws BizException
	 */
	public FMap getMeasDocMap() throws BizException {
	...
	public FMap getRouteMap() throws DAException {
