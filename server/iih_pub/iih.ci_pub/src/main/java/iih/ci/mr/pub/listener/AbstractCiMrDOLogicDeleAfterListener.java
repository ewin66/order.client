...
public class AbstractCiMrDOLogicDeleAfterListener implements IBusinessListener {
	...
	public void doAction(IBusinessEvent arg0) throws BizException {
		...
		CiMrDO[] ciMrDOsOld = (CiMrDO[])((BDCommonEvent)arg0).getOldObjs();
		
		CiMrDO newCiMr = ciMrDOsNew[0];
		CiMrDO oldCiMr = ciMrDOsOld[0];
		
		if(newCiMr.getDs()==1&&oldCiMr.getDs() ==0)
		{
			...
			//所有需要删除的引用
			MrDocRefValueDO[] MrDocRefValueDOs = docRefRService.find("id_mr = '"+oldCiMr.getId_mr()+"'", null, FBoolean.FALSE);
