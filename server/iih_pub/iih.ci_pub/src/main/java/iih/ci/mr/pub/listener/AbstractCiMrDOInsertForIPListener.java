...
public class AbstractCiMrDOInsertForIPListener extends AbstractCiMrDOInsertAfterListener{

	@Override
	protected void doActionCiMrDOInsert(CiMrDO[] ciMrDOsNew)
			throws BizException {
		...
		// 文书集成平台dto服务
				...
				//填充其他信息 这里是引用 知情同意书从这取，第一次 新增后  引用表的idmr暂存ident，真正保存mr后再更新成新的真正的idmr
				IMrdocrefvalueRService mrdocrefvalueservice = ServiceFinder.find(IMrdocrefvalueRService.class);
				MrDocRefValueDO[] mrDocRefValueDOs = mrdocrefvalueservice.find("id_mr = '"+ ciMrIPBaseDTO.getId_en() +"' and id_ent = '"+ ciMrIPBaseDTO.getId_en() +"'","",FBoolean.FALSE);
				...
				SecInfoDTO[] secInfoDtos = service.getSecInfoDTOForInsertAfter(ciMrDO.getId_ent());
